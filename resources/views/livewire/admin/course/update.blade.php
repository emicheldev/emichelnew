            ...
            <div class="p-6 bg-white rounded-md shadow-md">
                ...
                <form wire:submit.prevent="update">
                    ...
                    <div class="grid grid-cols-1 gap-6 my-4 sm:grid-cols-2">
                        <label class="block">
                            <select class="block w-full mt-1 form-select" wire:model="category_id">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                        <div>
                            <input class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="text" placeholder="Title of post" wire:model='title'>
                            @error('title') <div  class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div>
                            <input class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="text" placeholder="url of post" wire:model='image'>
                            @error('image') <div  class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                    </div>

                    <label class="block my-5">
                        <textarea class="block w-full mt-1 form-textarea" rows="3" placeholder="Enter description of post." wire:model='description'></textarea>
                        @error('description') <div  class="invalid-feedback">{{ $message }}</div> @enderror

                    </label>
                    <label class="block">
                        <textarea class="block w-full mt-1 form-textarea" rows="5" placeholder="Enter introduction." wire:model='introduction'></textarea>
